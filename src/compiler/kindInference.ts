...
// Use TypeScript's built-in types instead of custom types.js
type Symbol = ts.Symbol;
type Type = ts.Type;
type TypeFlags = ts.TypeFlags;
type TypeParameter = ts.TypeParameter;
...
type TypeParameterDeclaration = ts.TypeParameterDeclaration;
type HeritageClause = ts.HeritageClause;
type ExpressionWithTypeArguments = ts.ExpressionWithTypeArguments;
type TypeReferenceNode = ts.TypeReferenceNode;
...
export function inferKindFromImplementation(
    ...
    const implementsClause = heritageClauses.find(clause => clause.token === ts.SyntaxKind.ImplementsKeyword);
    if (!implementsClause) {
        return undefined;
    }
...
export function tryInferAndAttachKindMetadata(
    ...
    // Note: getSymbolLinks is internal to TypeScript, use a safe alternative
    const symbolAny = symbol as any;
    if (symbolAny.kindArity !== undefined) {
