...
// The above is false - this is being used
/**
...
/**
 * Multiply two usage bounds together
 */
export function combineUsageBounds<T>(
  a: UsageBound<T>,
  b: UsageBound<T>
): UsageBound<T> {
  return {
    usage: (input: T) => {
      const aUsage = a.usage(input);
      const bUsage = b.usage(input);
      if (aUsage === "∞" || bUsage === "∞") return "∞";
      return (aUsage as number) * (bUsage as number);
    },
    maxUsage: a.maxUsage === "∞" || b.maxUsage === "∞" ? "∞" : 
             (a.maxUsage || 1) * (b.maxUsage || 1)
  };
}

/**
 * Constant usage bound for infinite usage
 */
export const infiniteBound = infinite;
...
export function multiplyUsageBounds<B>(
  outer: UsageBound<B>,
  inner: UsageBound<B>
): UsageBound<B> {
  return {
...
export type WithMaxUsage<
  Stream extends UsageBoundStream<unknown, unknown, unknown>,
 ...
 */
export function isUsageBoundStream(
  value: unknown
): value is UsageBoundStream<unknown, unknown, unknown> {
  ...
  return (
    ...
    v.__brand === 'StatefulStream' &&
    typeof v.__purity === 'string' &&
    typeof v.usage === 'function'
  );
}
 ...
 */
export function hasMaxUsage(stream: UsageBoundStream<unknown, unknown, unknown>): stream is UsageBoundStream<unknown, unknown, unknown> & { maxUsage: Multiplicity } {
